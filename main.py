...
def summ_AllVideos(inputpath, K, T, H, it, n, s):
    videos = [f for f in listdir(getVideosInputFolder(inputpath)) if isfile(join(getVideosInputFolder(inputpath), f))]
    for x in videos:
...
def summ_Video(inputpath, K, T, H, it, n, s, videoName):
    framesfolder = getFramesFolder(inputpath, videoName)
    ...
    if (len(image_names) < 3): #Frames already generated, start algorithm
        video_to_frames(getVideosInputFolder(inputpath) + "/" + videoName, framesfolder)
    ...
    keyframes = write_keyframes(name_img, name_histr, centroids, classified_images, getKeyframesResultFolder(inputpath, videoName))

    # With the keyframes we reconstruct the summary video and write it in outpath
    if not os.path.isdir(getVideosResultFolder(inputpath)):
        os.makedirs(getVideosResultFolder(inputpath))
    write_video(keyframes, framesfolder, s, name_histr, name_img, image_names, H, classified_images, getVideosResultFolder(inputpath) + "/summarized-" + videoName)
...
def getKeyframesResultFolder(input, videoName):
    return input + "/resultFrames/" + videoName

def getVideosResultFolder(input):
    return input + "/resultVideos"

def getVideosInputFolder(input):
    return input

def getFramesFolder(input, videoName):
    return getFramesGeneralFolder(input) + "/" + videoName

def getFramesGeneralFolder(input):
    return input + "/frames"

def createOrClearFolder(folder):
...
def write_video(keyframes, input, S, name_histr, name_img, names, H, classified_images, outputFile):
    ...
    if os.path.isfile(outputFile):  # If the result video exist. Remove
        os.remove(outputFile)
    # Initialize the video writer
    video = cv2.VideoWriter(outputFile + ".avi", fourcc, 30.0, (width, heigth))
